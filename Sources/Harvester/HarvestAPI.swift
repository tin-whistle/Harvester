...
public class HarvestAPI {
    ...
    public func getProjectAssignments() async throws -> [HarvestProjectAssignment] {
        let response: UserProjectAssignmentsRequest.Response = try await networkClient.send(
            UserProjectAssignmentsRequest(userID: "me"))
        return response.projectAssignments
    }

    public func getTimeEntries() async throws -> [HarvestTimeEntry] {
        let response: TimeEntriesRequest.Response = try await networkClient.send(
            TimeEntriesRequest())
    ...
    public func startTimeEntryWith(
        ...
        try await networkClient.send(
            StartTimeEntryRequest(
                hours: hours, notes: notes, projectId: projectId, spentDate: spentDate,
                taskId: taskId))
    }
    ...
    public func deleteTimeEntry(_ timeEntry: HarvestTimeEntry) async throws {
        ...
        do {
            let _: HarvestTimeEntry = try await networkClient.send(
    ...
    public init(
        configuration: HarvestAPIConfiguration,
